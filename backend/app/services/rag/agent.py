...
- tools 参数预留：当前为空（过程式路由），未来可注册 @tool 实现自主 tool-calling
"""
...
# 预留：未来注册的外部工具列表
# 当前为空——检索策略路由在 pipeline 层过程式调用
# 未来可注册 web_search / api_query 等 @tool 实现 ReAct 式自主调用
_external_tools: list[Any] = []
...
def register_tool(tool: Any) -> None:
    ...
    Args:
        tool: PydanticAI @tool 装饰的函数
    """
    _external_tools.append(tool)
    # 重置 agent 单例，下次调用时重建（加载新 tools）
    rag_agent._agent = None
    logger.info("tool_registered", tool=getattr(tool, "__name__", str(tool)))
...
class RAGAgent:
    ...
    tools 参数预留扩展点：
    - 当前：tools=None（过程式路由，pipeline 层按 route 调用检索）
    - 未来：tools=_external_tools（LLM 自主 tool-calling，接近 ReAct）
    """
    ...
    def _get_agent(self) -> Agent:
        ...
        if self._agent is None:
            ...
            # 预留 tools 注册点：有外部工具时传入
            tools = _external_tools if _external_tools else None
            self._agent = Agent(model=model, tools=tools)
            logger.info(
                "rag_agent_created",
                tools_count=len(_external_tools),
            )
        return self._agent
