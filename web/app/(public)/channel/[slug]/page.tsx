...
import { mockPosts, mockChannels } from '@/lib/mock';

export function generateStaticParams() {
  return mockChannels.map((c) => ({ slug: c.slug }));
}
...
export function generateMetadata({
  ...
  const c = mockChannels.find((x) => x.slug === params.slug);
  return { title: c?.name ?? '频道' };
}
...
export default function ChannelPage({
  ...
  const channel = mockChannels.find((c) => c.slug === params.slug);
  if (!channel) notFound();
  const posts = mockPosts.filter((p) => p.channel === channel.slug);
